...
The official LST20 corpus is downloaded manually from AI for Thai.  This module
reads the extracted train/eval/test text files directly instead of depending on
the historical Hugging Face dataset loading script.
"""
...
def parse_lst20_file(path: str | Path) -> list[dict]:
    """Parse one LST20 .txt file into sentence-level examples."""
    path = Path(path)
    examples: list[dict] = []
    tokens: list[str] = []
    pos_tags: list[str] = []
    ...
    with path.open(encoding="utf-8") as handle:
        for raw_line in handle:
            ...
            ner_tags.append(ner_tag if ner_tag in VALID_NER_TAGS else "O")
...
def load_lst20_dataset_dict(
    ...
    return DatasetDict(
        {
            "train": Dataset.from_list(load_lst20_split(data_dir, "train", train_limit)),
            "validation": Dataset.from_list(
                load_lst20_split(data_dir, "validation", validation_limit)
            ),
            "test": Dataset.from_list(load_lst20_split(data_dir, "test", test_limit)),
        }
    )
